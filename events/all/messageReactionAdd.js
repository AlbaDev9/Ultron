...
if(reaction.emoji.name === "⭐"){
 ...
 if (reaction.emoji.name === '🎫' && reaction.message.id === data.MessageID) {
     ...
     const successEmbed = new MessageEmbed()
         ...
         .setDescription(`Ce ticket à été crée par ${user.toString()}. Merci de dire \`stop\` pour fermer le ticket et \`close\` pour le supprimé`)
         .setColor('BLUE');
...
async function checkIfClose(client, reaction, user, successMsg, channel) {
    ...
    collector2.on('collect', async msg => {
        channel.send(`Ce ticket à été fermé.`);
 ...
 collector.on('collect', async msg => {
     channel.send(`Ce ticket vas se supprimé dans **3** secondes`);
