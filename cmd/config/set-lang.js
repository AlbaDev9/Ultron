...
if(Fonctions.lang == "en"){
    if(message.author.id !== message.guild.ownerID) return message.channel.send(new MessageEmbed().setTitle("Order to reserve to the server owner").setDescription(`This command is reserved at <@${message.guild.ownerID}>! \nPlease do this command, please do it in a server where you are the owner` )).then(async message => {message.delete({timeout : 5000})})
         const embed = new MessageEmbed().setTitle('What do you want to do ?').setDescription("`FR`, `EN` please answer if below ?")
         const author = message.author.id
 message.channel.send(embed)
}
