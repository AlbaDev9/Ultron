...
module.exports = async (client, member) => {
    ...
    if(infos.activate === "on"){
        member.kick().then((member) => {member.send('The server is in raid mode ... Please join again later')})
    }
...
if(datas.logs === undefined){
    ...
    .setTitle('A member has just joined')
    .setDescription(`${member.name} just joined `)
