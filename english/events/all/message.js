...
module.exports = async (client, message) => {
    ...
    const prefixembed = new MessageEmbed()
        .setTitle("Prefix of this server")
        .setDescription(`The prefix is ${prefix}`)
        .setFooter(`To change it, please do ${prefix}setprefix`)
...
const Warning = require('../../db/schema/warns')
     if(fonctions.insulte === "on"){
        let blacklisted = ['con'  ,'fuck', 'fdp', 'ntm', 'connard', 'pute', 'putain', 'ta gueule', 'nique', 'salope', 'PD', 'batard', 'putin', 'enfoiré', 'connare', 'fils de pute', 'bâtard', 'bicot', 'conasse', 'couille molle', 'débile', 'ducon', 'dugland', 'enculé', 'fachiste', 'imbécile', 'lavette']
        ...
        if (foundInText) {
            ...
            message.channel.send(new MessageEmbed()
                .setDescription(`${message.author} you were warned by for the reason: ** That you sent an insult **`)
                .setColor("RED")
            )
        }
       ...
       if(fonctions.antilink === "on"){
           ...
           if(message.content.includes('discord.gg/')) {
               ...
               if(!IsOurInvite){
                ...
                message.channel.send(new MessageEmbed()
                    .setDescription(`${message.author} you have been notified by for the reason: ** That you have uploaded a link to another server **`)
                    .setColor("RED")
                )
               }
    ...
    if (command.help.args && !args.length) {
        const arg = new MessageEmbed()
            ...
            .setColor('RED')
            .setDescription(`We need more precision!`)
        let NoArgsReply = arg
        const noPerm = new MessageEmbed()
            .setTitle('`🚫`・You don\'t have the perms')
            .setColor('RED')
        ...
        const argS = new MessageEmbed()
            .setTitle('`❓`・We don\'t have enough information')
            .setColor('RED')
            .setDescription(`We need more precision! \nHere is how the command is used: \`${prefix} ${command.help.name} ${command.help.usage} \``)
        ...
        if (tStamps.has(message.author.id)) {
            ...
            if (timeNow < cdExpiration) {
                ...
                const notime = new MessageEmbed()
                    .setTitle('`🕒`・ A bit of patience')
                    .setColor('RED')
                    .setDescription(`Please wait \`${timeLeft.toFixed (0)} \` second (s)!`)
    ...
    const noUser = new MessageEmbed()
        .setTitle('`🚫`・ No user')
        .setColor('RED')
        .setDescription("You did not notice a user! Thanks for doing it")
    const UserAdmin = new MessageEmbed()
        .setTitle('`👮‍♂️`・This person is from the Staff')
        .setColor('RED')
        .setDescription("You cannot use this command against this person!")
   ...
   if(command.help.botPerms === true && command.help.permissions){
    ...
    message.channel.send(new MessageEmbed() .setTitle('I don\'t have certain permissions') .setDescription(`To make the command \`${command.help.name} \` I need the perm (s) \`${command.help.permissions} \``))
   }
    ...
    if (!message.member.hasPermission(command.help.permissions || [])) {
        ...
        .setTitle('`🚫`・You don\'t have the perms')
        .setColor('RED')
        .setDescription(`This command requires permissions : \`${command.help.permissions || []}\``)
        return message.channel.send(NoPerms)
    }
    if(command.help.premium === true && !(await Premium.findOne({GuildID : message.guild.id}))) return message.channel.send(new MessageEmbed().setTitle('Premium mode required'))
